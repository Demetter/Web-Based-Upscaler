...
def estimate_vram_and_tile_size(model, input_size, initial_tile_size=None):
    """
    Estimate VRAM requirements and adjust tile size if needed.
    Now supports automatic tile size calculation.
    """
    # Match the config format (lowercase keys)
    use_auto_tile_size = config['Processing'].get('autotilesize', 'false').lower() == 'true'
    
    # Get initial tile size
    if initial_tile_size is None:
        if use_auto_tile_size:
            initial_tile_size = calculate_optimal_tile_size(input_size[0], input_size[1], model.scale)
            logger.debug(f"Using auto tile size: {initial_tile_size}")
        else:
            initial_tile_size = int(config['Processing'].get('defaulttilesize', 384))
            logger.debug(f"Using default tile size: {initial_tile_size}")
    ...
    # Just add this log line before the return statement
    logger.info(f"Image size {input_size}, using tile size: {adjusted_tile_size}")
    
    return estimated_vram, adjusted_tile_size
